...
if(isset($_POST['Submit'])) {	
	...
	if(empty($firstName) || empty($lastName) || empty($address) || empty($email) || empty($city) || empty($division) || empty($zipcode) || empty($comments) || empty($age) || empty($salary)) {
		...
		//previous page
		echo "<br/><a class='btn btn-danger text-decoration-none' href='javascript:self.history.back();'>Go Back</a>";
	} else { 
			
		//insert data	
		$result = mysqli_query($mysqli, "INSERT INTO customers(FirstName,LastName,Address,Email,City,	 Division,Zipcode,Commnets,Age,Salary)
		VALUES('$firstName', '$lastName', '$address', '$email', '$city', '$division', '$zipcode','$comments', '$age', '$salary') ");
		
		//display success message
		echo "<div class='mt-5 mb-3 ml-3'><span class='alert alert-success'>Data Added Successfully.</span></div>";
